# Aprendizaje por Refuerzo Activo - Ejemplo básico
...
# Definir el entorno de recompensas (simple ejemplo)
...
def Q_learning(Q, rewards, gamma, alpha, epsilon, n_episodes=100):
    for _ in range(n_episodes):
        state = np.random.randint(0, n_states)  # Estado aleatorio inicial
        done = False
        while not done:
            action = epsilon_greedy(Q, state, epsilon)
            next_state = (state + 1) % n_states  # Transición aleatoria para el ejemplo
            reward = rewards[state][action]
            Q[state, action] += alpha * (reward + gamma * np.max(Q[next_state]) - Q[state, action])
            state = next_state
            done = np.random.rand() < 0.1  # Probabilidad de finalización de episodio
    return Q
